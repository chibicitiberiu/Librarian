...
BrowseController.prototype.setFileSelected = function (fileEntry, value) {
    return fileEntry.querySelector("#chk-selected").checked = value;
}

BrowseController.prototype.setFileCut = function (fileEntry, isCut) {
    if (isCut) {
...
BrowseController.prototype.setFileCopied = function (fileEntry, isCopied) {
    if (isCopied) {
...
BrowseController.prototype.selectAll = function () {
    for (var i = 0; i < this.fileEntries.length; i++) {
        this.setFileSelected(this.fileEntries[i], this.chkSelectAll.checked);
    }
}

BrowseController.prototype.getSelectedFiles = function () {
    var selection = [];
    for (var i = 0; i < this.fileEntries.length; i++) {
...
BrowseController.prototype.getSelectedFileIds = function () {
    var selection = [];
    for (var i = 0; i < this.fileEntries.length; i++) {
        if (this.getFileSelected(this.fileEntries[i])) {
...
BrowseController.prototype.onClipboardChanged = function (operation, newItems) {
...
BrowseController.prototype.cut = function () {
...
BrowseController.prototype.onCutCompleted = function (request) {
...
BrowseController.prototype.copy = function () {
    var selectedItems = this.getSelectedFileIds();
    if (selectedItems.length == 0)
        return;
...
BrowseController.prototype.onCopyCompleted = function (request) {
    if (request.readyState != 4)
        return;
...
BrowseController.prototype.paste = function () {
...
BrowseController.prototype.onPasteCompleted = function (request) {
    if (request.readyState != 4)
        return;
...
BrowseController.prototype.findItemColumnIndex = function (columnName) {
...
BrowseController.prototype.rename = function () {
...
BrowseController.prototype.renameSingleItem = function (itemToRename) {
    var _this = this;
...
BrowseController.prototype.onTxtRenameKeyUp = function (sender, event) {
    if (event.key == "Enter") {
...
BrowseController.prototype.onTxtRenameLostFocus = function (sender, event) {
...
BrowseController.prototype.completeRenameOperation = function () {
...
BrowseController.prototype.onCompleteRenameOperationCompleted = function (request) {
    if (request.readyState != 4)
        return;
...
BrowseController.prototype.cancelRenameOperation = function () {
    // find column
    var nameColIdx = this.findItemColumnIndex("File");
    if (!nameColIdx)
        nameColIdx = this.findItemColumnIndex("Name");
...
BrowseController.prototype.delete = function () {
    var payload = {
        "path": document.librarian_browse_path,
...
BrowseController.prototype.editMetadata = function () {
    var url = document.librarian_browse_urlMetadata + "/" + this.getSelectedFileIds()[0];
    location.href = url;
}

BrowseController.prototype.onCheckboxClicked = function () {
...
function browse_setup() {
    document.librarian_browse_BrowseController = new BrowseController();
}

window.addEventListener("load", browse_setup);